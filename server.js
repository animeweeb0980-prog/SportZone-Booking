...
app.post('/api/bookings', (req, res) => {
  ...
  if (!court_id || !user_name || !user_email || !date || !time_slot) return res.status(400).json({ error: 'All fields required' });
...
app.get('/api/leaderboard', (req, res) => {
  ...
  const leaders = sport
    ...
    : db.prepare('SELECT b.user_name, b.user_email, COUNT(*) as total_sessions FROM bookings b WHERE b.status = "confirmed" GROUP BY b.user_email ORDER BY total_sessions DESC LIMIT 20').all();
  res.json(leaders);
});
