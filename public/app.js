...
var emojis = {tennis:'🎾',pickleball:'🏓',badminton:'🏸',football:'⚽',volleyball:'🏐',basketball:'🏀'};
var colors = ['#ef4444','#f97316','#eab308','#22c55e','#3b82f6','#8b5cf6','#ec4899','#14b8a6'];

window.onload = function() {
  document.getElementById('bdate').onchange = loadSlots;
  try {
...
async function doRegister() {
  ...
  try {
    var res=await fetch('/api/register',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name,email,password:pass})});
    var d=await res.json();
    if(d.success){currentUser=d.user;localStorage.setItem('sportzone_user',JSON.stringify(currentUser));showApp();}
...
async function loadHome() {
  try {
    var bookings=await fetch('/api/bookings?email='+currentUser.email).then(r=>r.json());
    ...
    var total=bookings.filter(b=>b.status==='confirmed').length;
    stats.innerHTML='<div class="stat-card"><span class="stat-num">'+total+'</span><span class="stat-lbl">Sessions</span></div>';
    Object.keys(sports).slice(0,2).forEach(function(s){
      stats.innerHTML+='<div class="stat-card"><span class="stat-num">'+sports[s]+'</span><span class="stat-lbl">'+s+'</span></div>';
    });
    var events=await fetch('/api/events').then(r=>r.json());
...
function quickBook(sport) {
  ...
  setTimeout(function(){
    filterCourt(sport, document.querySelector('.chip[onclick*="'+sport+'"]'));
  }, 100);
}
...
async function loadCourts(sport) {
  ...
  try {
    var courts=await fetch(url).then(r=>r.json());
    if(!courts.length){grid.innerHTML='<p style="padding:40px;text-align:center;color:#9ca3af">No courts found.</p>';return;}
    grid.innerHTML=courts.map(function(c){
      var emoji=emojis[c.sport]||'🏟️';
      return '<div class="court-card">'+
        '<div class="cc-top"><div class="cc-emoji em-'+c.sport+'">'+emoji+'</div><span class="sp-tag st-'+c.sport+'">'+c.sport+'</span></div>'+
        '<div class="cc-body"><h3>'+c.name+'</h3><div class="loc">📍 '+c.location+'</div><p>'+c.description+'</p>'+
        '<div class="price-tag">₹'+c.price_per_hour+'/hour</div>'+
        '<button class="book-btn" data-id="'+c.id+'" data-name="'+c.name+'" data-sport="'+c.sport+'" data-loc="'+c.location+'">Book Now →</button></div></div>';
    }).join('');
    document.querySelectorAll('.book-btn').forEach(function(btn){
      btn.onclick=function(){openModal(this.dataset.id,this.dataset.name,this.dataset.sport,this.dataset.loc);};
    });
...
function openModal(id, name, sport, loc) {
  selectedCourtId=id; selectedSlot=null;
  ...
  document.getElementById('slots').innerHTML='<p class="shint">Choose a date to see available times</p>';
...
function closeModal() {
  ...
  selectedCourtId=null; selectedSlot=null;
}

async function loadSlots() {
  var date=document.getElementById('bdate').value;
  if(!date||!selectedCourtId) return;
  document.getElementById('slots').innerHTML='<p class="shint">Loading...</p>';
  try {
    var slots=await fetch('/api/availability?court_id='+selectedCourtId+'&date='+date).then(r=>r.json());
    document.getElementById('slots').innerHTML=slots.map(function(s){
      return s.available?'<button class="slot-btn" data-time="'+s.time+'">'+s.time+'</button>':'<button class="slot-btn taken" disabled>'+s.time+'</button>';
    }).join('');
  ...
  } catch(e){document.getElementById('slots').innerHTML='<p class="shint" style="color:#ef4444">Error loading slots.</p>';}
}
...
async function confirmBooking() {
  ...
  try {
    var res=await fetch('/api/bookings',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({court_id:selectedCourtId,user_name:currentUser.name,user_email:currentUser.email,date,time_slot:selectedSlot})});
    var d=await res.json();
    if(d.success){
      closeModal();
      document.getElementById('success-msg').textContent='Booking #'+d.booking_id+' — '+date+' at '+selectedSlot;
...
async function loadBookings() {
  ...
  try {
    var bks=await fetch('/api/bookings?email='+currentUser.email).then(r=>r.json());
    ...
    list.innerHTML=bks.map(function(b){
      ...
      return '<div class="bk-card"><div class="bk-left"><div class="bk-icon">'+emoji+'</div><div><div class="bk-sport">'+b.sport+'</div><h4>'+b.court_name+'</h4><div class="bk-time">📅 '+b.date+' · ⏰ '+b.time_slot+'</div><div class="bk-loc">📍 '+b.location+'</div></div></div><div style="display:flex;flex-direction:column;align-items:flex-end;gap:8px"><span class="status-badge '+statusClass+'">'+b.status+'</span>'+cancelBtn+'</div></div>';
    }).join('');
    document.querySelectorAll('.cancel-bk-btn').forEach(function(btn){
      btn.onclick=async function(){
        if(!confirm('Cancel this booking?')) return;
        await fetch('/api/bookings/'+this.dataset.id,{method:'DELETE'});
        loadBookings();
      };
    });
  ...
  try {
    ...
    list.innerHTML=events.map(function(e){
      ...
      return '<div class="ev-card">'+
        '<div class="ev-top"><div><div class="ev-title">'+emoji+' '+e.title+'</div><span class="sp-tag st-'+e.sport+'">'+e.sport+'</span></div><span style="font-size:0.8rem;font-weight:600;color:'+(spots>0?'#16a34a':'#ef4444')+'">'+(spots>0?spots+' spots left':'Full')+'</span></div>'+
        '<p class="ev-desc">'+e.description+'</p>'+
        '<div class="ev-meta"><span>📅 '+e.event_date+'</span><span>📍 '+e.location+'</span><span>👥 '+e.registered_count+'/'+e.max_participants+'</span></div>'+
        '<div class="ev-foot"><span class="reg-count">'+e.registered_count+' registered</span>'+
        (spots>0?'<button class="reg-btn" data-id="'+e.id+'">Register →</button>':'<button class="reg-btn registered">Full</button>')+
        '</div></div>';
    }).join('');
    document.querySelectorAll('.reg-btn:not(.registered)').forEach(function(btn){
      btn.onclick=async function(){
        var res=await fetch('/api/events/'+this.dataset.id+'/register',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({user_email:currentUser.email,user_name:currentUser.name})});
        var d=await res.json();
...
async function loadLeaderboard(type, btn) {
  if(btn){document.querySelectorAll('.lb-tabs .chip').forEach(b=>b.classList.remove('active'));btn.classList.add('active');}
  ...
  try {
    ...
    if(type!=='overall') url='/api/leaderboard';
    var leaders=await fetch(url).then(r=>r.json());
    if(type!=='overall') leaders=leaders.filter(l=>l.sport===type);
    ...
    var medals=['🥇','🥈','🥉'];
    table.innerHTML='<div class="lb-table">'+leaders.map(function(l,i){
      var color=colors[i%colors.length];
      return '<div class="lb-row"><span class="lb-rank">'+(medals[i]||('#'+(i+1)))+'</span>'+
        '<div class="lb-av" style="background:'+color+'">'+l.user_name.charAt(0).toUpperCase()+'</div>'+
        '<div style="flex:1"><div class="lb-name">'+l.user_name+'</div><div class="lb-email">'+l.user_email+'</div></div>'+
        '<div class="lb-score">'+l.total_sessions+'<span>sessions</span></div></div>';
    }).join('')+'</div>';
...
function loadLocations() {
  var locations=[
    {icon:'🏟️',name:'Sports Complex',desc:'Main sports hub with Tennis, Badminton and Volleyball courts.',sports:['Tennis','Badminton','Volleyball'],address:'Block A & B, Main Road'},
    {icon:'🎯',name:'Recreation Center',desc:'Dedicated facility for Pickleball and casual sports.',sports:['Pickleball'],address:'Recreation Park, Sector 4'},
    ...
    {icon:'🏠',name:'Indoor Stadium',desc:'Climate-controlled indoor courts for all weather play.',sports:['Badminton','Volleyball'],address:'Indoor Stadium, Hall 1 & 2'},
    {icon:'🌊',name:'Outdoor Arena',desc:'Beach volleyball and outdoor sports facilities.',sports:['Volleyball'],address:'Outdoor Arena, Beach Road'}
  ];
  document.getElementById('locations-grid').innerHTML=locations.map(function(l){
    return '<div class="loc-card"><div class="loc-icon">'+l.icon+'</div><h3>'+l.name+'</h3><p>'+l.desc+'</p><p style="font-size:0.78rem;color:#9ca3af;margin-bottom:10px">📍 '+l.address+'</p><div class="loc-tags">'+l.sports.map(s=>'<span class="loc-tag">'+s+'</span>').join('')+'</div></div>';
  }).join('');
}