...
db.exec('CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, email TEXT UNIQUE NOT NULL, password TEXT NOT NULL, created_at DATETIME DEFAULT CURRENT_TIMESTAMP)');
...
if (eventCount.count === 0) {
  const ie = db.prepare('INSERT INTO events (title, sport, description, location, event_date, max_participants) VALUES (?, ?, ?, ?, ?, ?)');
  ie.run('Summer Tennis Open', 'tennis', 'Annual summer tennis tournament open to all skill levels!', 'Sports Complex', '2026-04-15', 32);
  ie.run('Pickleball Doubles Championship', 'pickleball', 'Exciting doubles tournament. Find a partner and sign up!', 'Recreation Center', '2026-04-22', 24);
  ie.run('Badminton League Week 1', 'badminton', 'Weekly badminton league. Round-robin format.', 'Indoor Stadium', '2026-04-10', 16);
  ie.run('5-a-side Football Cup', 'football', 'Team football tournament. Register your team of 5!', 'Turf Ground', '2026-05-01', 40);
  ie.run('Volleyball Beginner Clinic', 'volleyball', 'Free beginner clinic. Learn the basics!', 'Sports Hall', '2026-04-18', 20);
}
